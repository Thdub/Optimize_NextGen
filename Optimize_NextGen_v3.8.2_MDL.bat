...
:Check_Caption
	...
	set "Shell_Title=[97m]0;Optimize Next Gen v3.8.2 LITE[97m"
	echo %Shell_Title%
	cls
...
:Inferior_Build
	set "Shell_Title=[97m]0;Optimize Next Gen v3.8.2[97m"
	echo %Shell_Title%
...
::============================================================================================================
	...
	if "%Run%" == "First" (
		...
		if "%SecretMode%" == "Unlocked" (
			"%Tmp_Folder%Files\Utilities\NSudoC.exe" -U:P -ShowWindowMode:Hide "%Tmp_Folder%Optimize_NextGen_v3.8.2_MDL.bat" && exit /b
		) else (
			"%Tmp_Folder%Files\Utilities\NSudoC.exe" -U:P -ShowWindowMode:Show "%Tmp_Folder%Optimize_NextGen_v3.8.2_MDL.bat" && exit /b
		)
	)
...
:: Make PS Script
	@echo $host.ui.RawUI.WindowTitle = "Optimize Next Gen v3.8.2 | Powershell Script">>%scriptname%
...
:Clear_EventViewer_Logs
	for /f "tokens=*" %%G in ('wevtutil.exe el') do (call :Clear_EventViewer_Logs_Task "%%G")
	goto :Cleaning_Temp_Folder

:Clear_EventViewer_Logs_Task
	wevtutil.exe cl %1
	goto :eof
