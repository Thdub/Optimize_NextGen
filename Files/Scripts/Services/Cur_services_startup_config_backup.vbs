...
End Function
   ...
   sREGFile = objFSO.GetParentFolderName(objFile) & "\Current_services_saved_on_" & strNow & ".reg"
   sBATFile = objFSO.GetParentFolderName(objFile) & "\Current_services_saved_on_" & strNow & ".bat"

   Set r = objFSO.CreateTextFile (sREGFile, True)
   r.WriteLine "Windows Registry Editor Version 5.00"
   r.WriteBlankLines 1
   r.WriteLine ";Services Startup Configuration saved before optimization at " & Time & " on " & FormatDateTime(Now, vbShortDate)
   r.WriteBlankLines 1
   ...
   b.WriteBlankLines 1
   b.WriteLine "echo ]0;Import initial services startup configuration, saved at " & Time & " on " & FormatDateTime(Now, vbShortDate) & ""
