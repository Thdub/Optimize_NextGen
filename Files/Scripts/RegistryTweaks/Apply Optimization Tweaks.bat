...
:: Set window Title
	echo ]0;Apply Optimization Tweaks
:: Set variable
...
::===============================================================================================================
	...
	<nul set /p dummyName=%BS%  -Registry: 
...
:: Do not allow Windows to automatically connect to suggested open hotspots, to networks shared by contacts, and to hotspots offering paid services
	reg add "HKLM\SOFTWARE\Microsoft\Windows\WcmSvc\wifinetworkmanager\config" /v "AutoConnectAllowedOEM" /t REG_DWORD /d "0" /f >NUL 2>&1
...
:: Check Windows edition before adding Shutdown Event Tracker value
	echo %Win_Edition% | findstr /i /c:"Server" >nul && (
	goto :ShutDownReasonOn_Allowed
	) || (
	goto :Power_1
	)
:ShutDownReasonOn_Allowed
...
:: No "shortcut" text added to shortcut name when creating
...
:: Check Windows edition before adding DisableCAD value
	echo %Win_Edition% | findstr /i /c:"Server" >nul && (
	goto :DisableCAD_Allowed
	) || (
	goto :DisableCAD_Skipped
	)
:DisableCAD_Allowed
